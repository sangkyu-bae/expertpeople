...
import {getCookie} from "../../util/common/Cookies";
...
function FalseLogin(props) {
    ...
    const fetchSse = async () => {
        try {
            ...
            eventSource.addEventListener('newNotification', function (e) {
                setIsNotify(true);
            }, false);
            ...
            <ul className="nav-item flex">
                <li className="nav-items">
                    <Link to="/notification/new">
                        {
                            isNotify ?
                                <FontAwesomeIcon className="bell" icon={farBell} style={{color: 'blue'}}/> :
